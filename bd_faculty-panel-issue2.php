        ...
        else
        {
            while($row = mysqli_fetch_assoc($ans))
            {
                if($row['copies'] > 0)
                {
                    ...
                    $sql2 = 'select * from facultybook where employee_no = '.$employee_no.' ;';
                    ...
                    while($row2 = mysqli_fetch_assoc($ans2))
                    {
                        ...
                        ( $employee_no , $book_id2 , '$book_name2' , '$book_auther2' , NOW() , NOW() , $book_edition2 , '$publisher2' , $semester2);";
                        $ans3 = mysqli_query($conn,$sql3);

                        //change into studentbook or facultybook.

                        $new_no_book = $row2['no_books'] + 1;
                        $sql4 = 'update facultybook set no_books = '.$new_no_book.' where employee_no = '.$employee_no.' ;';
